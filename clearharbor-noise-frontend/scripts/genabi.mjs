...
const CONTRACT_NAME = "NoiseMonitor";

// Path to fhevm-hardhat-template (relative to this script)
...
if (!fs.existsSync(dir)) {
  // If hardhat-template doesn't exist, check if ABI files already exist
  ...
  if (fs.existsSync(abiFile) && fs.existsSync(addressesFile)) {
    console.log(`${line}Hardhat template not found, but ABI files exist. Skipping generation.${line}`);
    process.exit(0);
  }
